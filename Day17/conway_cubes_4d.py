...
class ConwayCubes:
    ...
    def __initialise(self, initial_input):
        ...
        for x_co_ordinate, row in enumerate(initial_input.splitlines()):
            for y_co_ordinate, cube in enumerate(row):
                if cube == "#":
                    cubes.add((x_co_ordinate, y_co_ordinate, 0, 0))
        return cubes
    ...
    def _are_neighbours(
        ...
        if cube_a == cube_b:
            return False
        a_x, a_y, a_z, a_w = cube_a
        b_x, b_y, b_z, b_w = cube_b
    ...
    def get_next_active_layout(self):
        ...
        min_x, max_x, min_y, max_y, min_z, max_z, min_w, max_w = self.playing_area()
